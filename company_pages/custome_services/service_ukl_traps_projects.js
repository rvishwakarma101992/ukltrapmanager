...
.constant('ENDPOINT_UKL_TRAPS_PROJECTS', 'http://localhost:5000/')
.service('service_ukl_traps_projects', ['$http', 'ENDPOINT_UKL_TRAPS', service_ukl_traps_projects])
;

function service_ukl_traps_projects($http, ENDPOINT_UKL_TRAPS){
	var service = this;
	
	var path = 'ukl_traps_projects/';
...
//	for create
	service.create = function(item,item_status){
		...
		return $http.post(get_url(), item);
			...
			// service_ukl_traps_projects.create(item)
			// .then(
			// 	function on_success(response){
			// 		swal({
			// 			title: "Success!", 
...
//	for get_one_by_id	
	service.fetch_by_id = function(item_id){
		...
		// return $http.get(get_url_for_id(item_id));
			...
			// service_ukl_traps_projects.fetch_by_id(item_id)
			// .then(
			// 	function on_success(response){
			//     	console.log("ON SUCCESS: " + response.data);
...
//	for delete_by_id
	service.delete_by_id = function(item_id){
		return $http.delete(get_url_for_id(item_id));
			...
			// service_ukl_traps_projects.delete_by_id(item_id)
			// .then(
			// 	function on_success(response){
			//     	console.log("ON SUCCESS: " + response.data);
			// 	}, 
...
//	for update_by_id
	service.update_by_id = function(item_id, item){
		...
		return $http.put(ENDPOINT_UKL_TRAPS + "ukl_traps_projects/" + "update_fully/" + item_id, item);
	};

			// service_ukl_traps_projects.update_by_id(item_id, item_object)
			// .then(
			// 	function on_success(response){
			// 		swal({
			// 			title: "Success!", 
